...
- [ ] ライブラリページの更新は削除されていたら更新しない
- [ ] ライブラリページの更新では「更新しますか(yes/no)」ダイアログを表示する
- [ ] ライブラリページの複数更新では更新間隔を設ける
- [ ] ライブラリページの更新はダイアログを表示して進行状況表示、キャンセルができるようにする
- [ ] nndd dbからの変換で項目サムネイルサイズ(S,L)を追加する(変換時はSにしておく)
- [ ] ライブラリページの画像更新でdbサムネイルサイズをLに更新する
- [ ] ライブラリの更新はNicoUpdateでjsonデータを保存するのでXMLDataConverterは不要？(削除済みなら更新ないので変換も不要？)
- [ ] ライブラリページでの表示サイズはdbサムネイルサイズで決定する
