...
const { NicoPlay } = require("../../lib/nico-play");
const { NicoUpdate } = require("../../lib/nico-update");
const { CommentFilter } = require("../../lib/comment-filter");
...
module.exports = {
    ...
    onBeforeMount() {
        ...
        player_obs.onReturn("player-main:get-current-play-video", () => {
            return this.current_play_video;
        });

        player_obs.on("player-main:update-data", async(video_id, update_target) => {
        ...
        player_obs.on("player-main:add-ng-comment", async (args) => {
            ...
            player_obs.trigger("player-info:update-comments", comments);
        });

        player_obs.on("player-main:delete-ng-comment", async (args) => {
            const { ng_texts, ng_user_ids } = args;
        ...
        player_obs.on("player-main:update-comment-display-limit", (args) => {
            const { do_limit } = args;
    ...
    async mouseup(e) { // eslint-disable-line no-unused-vars
        if(this.gutter_move){
            ...
            player_obs.trigger("player-info:split-resized");
