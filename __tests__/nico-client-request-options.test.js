...
test("options", async (t) => {
    ...
    const body = await req.options(test_url, {json:json_data});
    t.is(body, "ok");
});
...
test("options timeout", async (t) => {
    ...
    try {
        const req = new NicoClientRequest();
        await req.options(test_url, {json:json_data, timeout_msec:1*1000});
    } catch (error) {
...
test("options page not find", async t => {
    ...
    try {
        const req = new NicoClientRequest();
        await req.options(test_url, {json:json_data});
    } catch (error) {
        t.is(error.cancel, undefined);
        t.is(error.name, "Error");
