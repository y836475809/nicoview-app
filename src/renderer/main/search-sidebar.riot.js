...
const { searchItems } = require("../../lib/nico-search");
...
module.exports = {
    ...
    onBeforeMount() {
        ...
        this.obs_listview.on("item-dlbclicked", (
            ...
            main_obs.trigger("search:item-dlbclicked", item);
        });

        main_obs.on("search:sidebar:add-item", (
            /** @type {NicoSearchParamsItem} */ item) => {
            const items = [item];
            this.obs_listview.trigger("addList", { items });
        });

        main_obs.on("search:sidebar:reload-items", async () => {
